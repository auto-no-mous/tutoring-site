        ...
        <div v-if="step === 1" class="mt-4">
          ...
          <div v-else class="flex max-h-72 flex-wrap gap-2 overflow-y-auto">
            <button
              ...
              :key="date"
              type="button"
              class="rounded-md border border-slate-300 px-3 py-1.5 text-sm hover:border-brand-400 dark:border-slate-700"
              @click="pickDate(date)"
            >
        ...
        <div v-else-if="step === 3 && selectedSlot" class="mt-4 flex flex-col gap-3">
          ...
          <div class="flex gap-2">
            <button
              ...
              class="rounded-md bg-brand-500 px-4 py-2 text-sm text-white disabled:opacity-50"
              @click="confirm"
            >
              Подтвердить перенос
            </button>
