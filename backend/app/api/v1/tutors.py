...
from app.schemas.tutor import TutorCatalogItem, TutorProfileOut, TutorProfileUpdate, TutorPublicProfile
...
def _to_profile_out(profile, display_name: str, is_active: bool = True) -> TutorProfileOut:
    return TutorProfileOut.model_validate(profile, from_attributes=True).model_copy(
        update={"display_name": display_name, "is_active": is_active}
    )


@router.get("", response_model=list[TutorCatalogItem])
async def catalog(
    db: DbSession,
    subject_id: uuid.UUID | None = None,
    price_min: float | None = None,
    price_max: float | None = None,
) -> list[TutorCatalogItem]:
    items = await tutor_service.search_catalog(db, subject_id=subject_id, price_min=price_min, price_max=price_max)
    return [TutorCatalogItem(**item) for item in items]
...
async def get_my_profile(current_user: CurrentUser, db: DbSession) -> TutorProfileOut:
    ...
    return _to_profile_out(profile, current_user.display_name, current_user.is_active)
...
async def get_public_profile(tutor_id: uuid.UUID, db: DbSession) -> TutorPublicProfile:
    profile = await tutor_service.get_profile_by_id(db, tutor_id)
    user = await db.get(User, profile.user_id)
    avg_rating, reviews_count = await review_service.get_rating_summary(db, tutor_id)
    subject_rows = await subject_service.get_tutor_subjects(db, tutor_id)
